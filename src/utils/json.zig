...
/// Emit a start event
pub fn emitStart(file: []const u8, size: u64) !void {
    try writeEvent(.start, .{ .file = file, .size = size });
}
...
/// Emit a done event
pub fn emitDone(output: []const u8, hash: []const u8) !void {
    try writeEvent(.done, .{ .output = output, .hash = hash });
}
...
/// Emit a keygen result event
pub fn emitKeygen(public_key: []const u8, secret_key: []const u8) !void {
    try writeEvent(.keygen, .{ .public_key = public_key, .secret_key = secret_key });
}
...
test "write event with fields" {
    ...
    try writeEventTo(fbs.writer(), .start, .{ .file = "test.txt", .size = @as(u64, 1024) });
    const result = fbs.getWritten();

    try std.testing.expectEqualSlices(u8, "{\"event\":\"start\",\"file\":\"test.txt\",\"size\":1024}\n", result);
}
